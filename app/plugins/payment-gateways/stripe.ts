...
export default defineNuxtPlugin(() => {
  ...
  const applyStripePaymentIntent = (paymentIntent: { id: string; payment_method?: unknown; amount: number; currency: string; status: string }): boolean => {
    ...
    // key when a payment_intent/charge webhook arrives. Without it, the webhook can't find the
    // order to record the real charge ID, so a later manual refund ends up sending the
    // PaymentIntent ID as the charge to Stripe, which Stripe rejects ("No such charge").
    upsertOrderMeta('_stripe_intent_id', paymentIntent.id);
    ...
    // Deliberately not setting orderInput.value.transactionId here: WooCommerce's own refund
    // flow reads the order's transaction ID verbatim and sends it to Stripe as the `charge`
    // param, but our WPGraphQL PaymentIntent type has no field for the real charge ID
    // (only the PaymentIntent's own `pi_...` id, which Stripe's refund API rejects with "No
    // such charge"). Leaving it unset lets the WooCommerce Stripe gateway's own
    // payment_intent.succeeded/charge.succeeded webhook — which can now find this order via
    // the _stripe_intent_id meta above — fill in the correct ch_... transaction ID itself,
    // exactly as it would for a non-headless checkout.
    return true;
  };
