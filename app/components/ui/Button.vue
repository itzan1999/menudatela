...
interface Props {
  variant?: 'primary' | 'secondary' | 'danger' | 'ghost' | 'outline' | 'outline-charcoal';
  size?: 'sm' | 'md' | 'lg';
  loading?: boolean;
  to?: string;
  icon?: string;
...
const variantClasses = computed(() => {
  const variants = {
    ...
    // Editorial charcoal/sand/cream outline, inverted on hover. Shared by every
    // "outline" call-site in the checkout/cart flow so the palette lives in one place.
    'outline-charcoal':
      'rounded-none border border-[var(--color-sand)] bg-transparent text-[var(--color-charcoal)] shadow-none transition-all duration-300 hover:border-[var(--color-charcoal)] hover:bg-[var(--color-charcoal)] hover:text-[var(--color-cream)] focus:ring-[var(--color-charcoal)]/30 disabled:hover:border-[var(--color-sand)] disabled:hover:bg-transparent disabled:hover:text-[var(--color-charcoal)]',
  };
